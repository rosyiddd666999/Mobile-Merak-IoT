...
class IncubatorStatusCard extends StatelessWidget {
  ...
  const IncubatorStatusCard(
      {super.key, required this.status, required this.settings});

  @override
  Widget build(BuildContext context) {
    final suhuState = classifyTemp(
        status.suhuSekarang, settings.suhuMin, settings.suhuMax);
            ...
            Row(
              children: [
                _Item(
                  ...
                  label: 'Suhu · ${suhuState.label}',
                  state: suhuState,
                ),
                _Item(
                  icon: Icons.water_drop,
                  value: '${status.kelembapanSekarang.toStringAsFixed(0)}%',
                  label: 'Lembap · ${lembapState.label}',
                  state: lembapState,
                ),
                _Item(
